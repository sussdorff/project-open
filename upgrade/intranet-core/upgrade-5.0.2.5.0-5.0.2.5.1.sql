...
SELECT acs_log__debug('/packages/intranet-core/sql/postgresql/upgrade/upgrade-5.0.2.5.0-5.0.2.5.1.sql','');
...
COPY tmp_countries (iso, country, year, tld, iso_3166, note) FROM stdin;
...
EH	Western Sahara	1974		ISO 3166-2:EH	Previous ISO country name: Spanish Sahara
...
UM	United States Minor Outlying Islands	1986		ISO 3166-2:UM	
...
DECLARE
	...
	row			RECORD;
BEGIN
	FOR row IN select lower(iso) as iso, country from tmp_countries LOOP
	    select count(*) into v_count from country_codes where iso = row.iso;
	    select country_name into v_old_country from country_codes where iso = row.iso;

	    -- RAISE NOTICE 'iso=%, count=%, country=%, old_country=%', row.iso, v_count, row.country, v_old_country;
	    IF v_count = 1 THEN
		update country_codes set country_name = row.country where iso = row.iso;
	    ELSE
		insert into country_codes (iso, country_name) values (row.iso, row.country);
	    END IF;
	END LOOP;
