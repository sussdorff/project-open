...
SELECT acs_log__debug('/packages/intranet-timesheet2/sql/postgresql/upgrade/upgrade-4.0.3.3.2-4.0.3.3.3.sql','');

-- remove function, does not follow naming conventions
...
begin
	...
	FROM 	pg_proc 
	WHERE 	proname = 'im_absences_month_absence_type' 
		and proargnames = '{user_id,month,year,absence_type_id}'; 

        IF      0 != v_count
        THEN
		drop function im_absences_month_absence_type(integer,integer,integer,integer); 
                return 1;
        END IF;
        return 0;
